...
- Creating or deactivating an agent or sub-admin account requires the
  `users:manage` permission (admin always has it; a sub-admin only if
  granted one).
...
- **The cap is on deactivation, not creation.** Creating or deactivating an `agent`/`subadmin` account requires the `users:manage` permission — delegable to a sub-admin who's been granted it (Story 46's `requirePermission` mechanism; `admin` always passes). Deactivating an **existing `admin` account** (these exist via DB provisioning, so the roster/deactivate endpoints must still handle them) is a separate, narrower check: always `requireRole("admin")` regardless of permissions — a sub-admin holding `users:manage` cannot deactivate a higher-privileged account.
- The roster (`GET`) should list all three staff roles (`agent`, `admin`, `subadmin`) for visibility, even though `admin` is never a creation target here.
- Since this story (08 in plan sequence) executes before Story 46 (configure-roles-and-permissions, which builds the `RolePermissions` model and `requirePermission` middleware), this story cannot literally call `requirePermission("users:manage")` yet. Gate on `requireRole("admin")` for now with a code comment marking the spot Story 46 converts to `requirePermission("users:manage")` once the permission layer exists — do not block this story on Story 46 landing first, and do not build a throwaway partial permission model here. The admin-target deactivation check stays `requireRole("admin")` permanently — no TODO on that branch.
...
- `requireAuth`, `requireRole("admin")` for all endpoints in this story (see Extra notes — this story predates Story 46's `requirePermission` middleware, so the `users:manage` delegation is a follow-up conversion, not built here). Mark the conversion point with a comment so Story 46's executor finds it.
...
- Actually wiring `requirePermission("users:manage")` — the middleware doesn't exist until Story 46. This story gates on `requireRole("admin")` with a marked conversion point (see Extra notes).
