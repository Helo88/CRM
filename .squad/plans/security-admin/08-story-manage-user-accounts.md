...
- [ ] `UserRole` includes `"subadmin"` in both the TypeScript type and the Mongoose schema enum (`backend/src/models/User.ts` lines 3, 68).
...
- [ ] `frontend/messages/en.json` and `frontend/messages/ar.json` both have matching `AdminUsersList`/`NewStaffAccount` sections and the `Nav.accounts` key.
