...
// Relocated from frontend/app/admin/ticket-categories/page.tsx
// (sla-automation Story 25, Frontend Task 7a) — now a tab under the
// /admin/system-configuration shell (layout.tsx owns the sidebar/heading/
// tab strip), so this file renders only the tab's own content. No backend
// change: still GET /api/v1/ticket-categories, same permission gating.
export default async function AdminSystemConfigurationCategoriesPage({
  searchParams,
}: {
  ...
  if (!token) {
    if (hasRefreshToken && !_refreshed) {
      redirect("/api/session/refresh?next=/admin/system-configuration/categories");
    }
    redirect("/");
  }
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        {canCreate && (
          <Button asChild size="sm">
            <Link href="/admin/system-configuration/categories/new">{t("addCategory")}</Link>
          </Button>
        )}
      </div>
